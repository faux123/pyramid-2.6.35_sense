...
static int config(struct pcm *pcm)
{
	...
	if (!atomic_read(&pcm->out_prefill)) {
		...
		rc = q6asm_media_format_block_pcm(pcm->ac, pcm->sample_rate,
							pcm->channel_count);
		if (rc < 0)
			pr_aud_err("%s: CMD Format block failed\n", __func__);
				...
				unsigned long arg)
{
	...
	case AUDIO_SET_VOLUME: {
		pr_aud_info("%s: AUDIO_SET_VOLUME, vol %lu\n", __func__, arg);
		rc = q6asm_set_volume(pcm->ac, arg);
		if (rc < 0)
...
static int pcm_out_open(struct inode *inode, struct file *file)
{
	...
	pr_aud_info("[%s:%s] open\n", __MM_FILE__, __func__);
	pcm = kzalloc(sizeof(struct pcm), GFP_KERNEL);
	if (!pcm) {
		pr_aud_info("%s: Failed to allocated memory\n", __func__);
		return -ENOMEM;
	}
	...
	if (rc < 0) {
		pr_aud_err("%s: pcm out open failed for session %d\n", __func__,
			pcm->ac->session);
		rc = -EINVAL;
		goto fail;
	}
	...
	pr_debug("[%s:%s] open session id[%d]\n", __MM_FILE__,
					...
					size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		if (!rc) {
			pr_aud_err("%s: wait_event_timeout failed for session %d\n",
				__func__, pcm->ac->session);
			goto fail;
		}
		...
		if (atomic_read(&pcm->out_stopped) &&
					!atomic_read(&pcm->out_count)) {
			pr_aud_info("%s: pcm stopped out_count 0\n", __func__);
