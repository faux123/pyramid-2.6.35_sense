...
static int32_t pm8058_xoadc_calib_device(uint32_t adc_instance)
{
	struct pmic8058_adc *adc_pmic = pmic_adc[adc_instance];
...
static int __devinit pm8058_xoadc_probe(struct platform_device *pdev)
{
	...
	if (adc_pmic->adc_voter == NULL) {
		adc_pmic->adc_voter = msm_xo_get(MSM_XO_TCXO_D1, "pmic8058_xoadc");
...
static int pm8058_xoadc_suspend(struct platform_device *pdev,
				pm_message_t state)
{
	mutex_lock(&mlock);
	is_suspend = 1;
...
static int pm8058_xoadc_resume(struct platform_device *pdev)
{
	mutex_lock(&mlock);
	is_suspend = 0;
	mutex_unlock(&mlock);
...
static struct platform_driver pm8058_xoadc_driver = {
	...
	.suspend = pm8058_xoadc_suspend,
	.resume = pm8058_xoadc_resume,
	.driver = {
		.name = "pm8058-xoadc",
		.owner = THIS_MODULE,
	},
