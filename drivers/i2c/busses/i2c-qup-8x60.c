...
#define LEAD_GENERAL_I2C_SDA           (59)
#define LEAD_GENERAL_I2C_SCL           (60)

#define GSBI1_I2C_SDA					(35)
#define GSBI1_I2C_SCL					(36)
#define GSBI2_I2C_SDA					(39)
#define GSBI2_I2C_SCL					(40)
#define GSBI3_I2C_SDA					(43)
#define GSBI3_I2C_SCL					(44)
#define GSBI4_I2C_SDA					(47)
#define GSBI4_I2C_SCL					(48)
#define GSBI5_I2C_SDA					(51)
#define GSBI5_I2C_SCL					(52)
#define GSBI6_I2C_SDA					(55)
#define GSBI6_I2C_SCL					(56)
#define GSBI7_I2C_SDA					(59)
#define GSBI7_I2C_SCL					(60)
#define GSBI8_I2C_SDA					(64)
#define GSBI8_I2C_SCL					(65)
#define GSBI9_I2C_SDA					(68)
#define GSBI9_I2C_SCL					(69)
#define GSBI10_I2C_SDA					(72)
#define GSBI10_I2C_SCL					(73)
#define GSBI11_I2C_SDA					(104)
#define GSBI11_I2C_SCL					(103)
#define GSBI12_I2C_SDA					(116)
#define GSBI12_I2C_SCL					(115)


static uint32_t gsbi3_gpio_table[] = {
	GPIO_CFG(GSBI3_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI3_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(GSBI4_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI4_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi5_gpio_table[] = {
	GPIO_CFG(GSBI5_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI5_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(GSBI7_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI7_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi8_gpio_table[] = {
	GPIO_CFG(GSBI8_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI8_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi9_gpio_table[] = {
	GPIO_CFG(GSBI9_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI9_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static uint32_t gsbi12_gpio_table[] = {
	GPIO_CFG(GSBI12_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(GSBI12_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
qup_i2c_interrupt(int irq, void *devid)
{
	...
	if (status & I2C_STATUS_ERROR_MASK) {
		dev_err(dev->dev, "[QUP I2C Err] QUP: I2C status flags :0x%x, irq:%d\n",
			status, irq);
		err = status;
		/* Clear Error interrupt if it's a level triggered interrupt*/
		if (dev->num_irqs == 1) {
...
qup_set_read_mode(struct qup_i2c_dev *dev, int rd_len)
{
	uint32_t wr_mode = (dev->wr_sz < dev->out_fifo_sz) ?
				QUP_WR_BLK_MODE : 0;
	if (rd_len > 256) {
		dev_err(dev->dev, "[QUP I2C Err] HW doesn't support READs > 256 bytes\n");
...
static int
qup_i2c_config_gpio(struct qup_i2c_dev *dev, int *gpio_clk, int *gpio_dat)
{

	int ret = 0;

	switch (dev->adapter.nr) {
	case MSM_GSBI3_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI3_I2C_SCL;
		*gpio_dat = GSBI3_I2C_SDA;
		gpio_tlmm_config(gsbi3_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi3_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI4_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI4_I2C_SCL;
		*gpio_dat = GSBI4_I2C_SDA;
		gpio_tlmm_config(gsbi4_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi4_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI5_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI5_I2C_SCL;
		*gpio_dat = GSBI5_I2C_SDA;
		gpio_tlmm_config(gsbi5_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi5_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI7_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI7_I2C_SCL;
		*gpio_dat = GSBI7_I2C_SDA;
		gpio_tlmm_config(gsbi7_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi7_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI8_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI8_I2C_SCL;
		*gpio_dat = GSBI8_I2C_SDA;
		gpio_tlmm_config(gsbi8_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi8_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI9_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI9_I2C_SCL;
		*gpio_dat = GSBI9_I2C_SDA;
		gpio_tlmm_config(gsbi9_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi9_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI10_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI10_I2C_SCL;
		*gpio_dat = GSBI10_I2C_SDA;
		gpio_tlmm_config(gsbi10_gpio_table[0], GPIO_CFG_ENABLE);
		gpio_tlmm_config(gsbi10_gpio_table[1], GPIO_CFG_ENABLE);
		break;

	case MSM_GSBI12_QUP_I2C_BUS_ID:
		*gpio_clk = GSBI12_I2C_SCL;
		*gpio_dat = GSBI12_I2C_SDA;
	...
	default:
		ret = -EAGAIN;
		dev_err(dev->dev, "[QUP I2C Err] unknow adap id: %d\n", dev->adapter.nr);
	}

	dev_info(dev->dev, "qup_i2c_config_gpio, dev->adapter.nr: %d, gpio_clk: %d, gpio_dat: %d\n", dev->adapter.nr, *gpio_clk, *gpio_dat);
	return ret;
}
...
QUP_i2c_recover_bus_busy(struct qup_i2c_dev *dev)
{
	...
	dev_warn(dev->dev, "QUP_i2c_recover_bus_busy, QUP_I2C_STATUS = 0x%x\n", status);

	if (!(status & (I2C_STATUS_BUS_ACTIVE)) || (status & (I2C_STATUS_BUS_MASTER)))
		return 0;


	dev_warn(dev->dev, "QUP_i2c_recover_bus_busy, try to recover..\n");

	if (qup_i2c_config_gpio(dev, &gpio_clk, &gpio_dat))
		return -EAGAIN;
	dev_info(dev->dev, "QUP_i2c_recover_bus_busy, dev->adapter.nr: %d, gpio_clk: %d, gpio_dat: %d\n", dev->adapter.nr, gpio_clk, gpio_dat);
...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	while (rem) {
		...
		do {
			...
			if (!timeout) {
				...
				dev_err(dev->dev, "[QUP I2C Err] Transaction timed out, device = 0x%x\n", dev->msg->addr);
			...
			if (dev->err) {
				if (dev->err > 0 &&
					...
					dev_err(dev->dev,
					"[QUP I2C Err] I2C slave addr:0x%x not connected\n",
					dev->msg->addr);
				else if (dev->err < 0) {
					dev_err(dev->dev,
					"[QUP I2C Err] QUP data xfer error %d\n", dev->err);
					ret = dev->err;
					goto out_err;
				}
				ret = -dev->err;
		...
		if (ret) {
			dev_err(dev->dev,
				"[QUP I2C Err] Error waiting for write ready\n");
			goto out_err;
		}
	}
 ...
 out_err:
	...
	dev->pwr_timer.expires = jiffies + 1;
...
qup_i2c_probe(struct platform_device *pdev)
{
	...
	if (!pdata) {
		dev_err(&pdev->dev, "[QUP I2C Err] platform data not initialized\n");
		return -ENOSYS;
	}
	qup_mem = platform_get_resource_byname(pdev, IORESOURCE_MEM,
						"qup_phys_addr");
	if (!qup_mem) {
		dev_err(&pdev->dev, "[QUP I2C Err] no qup mem resource?\n");
		return -ENODEV;
	}
	...
	err_irq = platform_get_resource_byname(pdev, IORESOURCE_IRQ,
						"qup_err_intr");
	if (!err_irq) {
		dev_err(&pdev->dev, "[QUP I2C Err] no error irq resource?\n");
		return -ENODEV;
	}
	...
	qup_io = request_mem_region(qup_mem->start, resource_size(qup_mem),
					pdev->name);
	if (!qup_io) {
		dev_err(&pdev->dev, "[QUP I2C Err] QUP region already claimed\n");
		return -EBUSY;
	}
	if (!pdata->use_gsbi_shared_mode) {
		gsbi_mem = platform_get_resource_byname(pdev, IORESOURCE_MEM,
							"gsbi_qup_i2c_addr");
		if (!gsbi_mem) {
			dev_err(&pdev->dev, "[QUP I2C Err] no gsbi mem resource?\n");
			return -ENODEV;
		}
		gsbi_io = request_mem_region(gsbi_mem->start,
						...
						pdev->name);
		if (!gsbi_io) {
			dev_err(&pdev->dev, "[QUP I2C Err] GSBI region already claimed\n");
			return -EBUSY;
		}
	}
	...
	if (IS_ERR(clk)) {
		dev_err(&pdev->dev, "[QUP I2C Err] Could not get clock\n");
		ret = PTR_ERR(clk);
	...
	if (pdata->pclk != NULL) {
		...
		if (IS_ERR(pclk)) {
			dev_err(&pdev->dev, "[QUP I2C Err] Could not get pclock\n");
			ret = PTR_ERR(pclk);
			clk_put(clk);
			goto err_clk_get_failed;
		}
	...
	if (!(pdata->msm_i2c_config_gpio)) {
		dev_err(&pdev->dev, "[QUP I2C Err] config_gpio function not initialized\n");
		ret = -ENOSYS;
	...
	if (pdata->clk_freq <= 0 ||
		...
		dev_err(&pdev->dev, "[QUP I2C Err] clock frequency not supported\n");
		ret = -EIO;
		goto err_config_failed;
	}
	...
	if (dev->num_irqs == 3) {
		...
		if (ret) {
			dev_err(&pdev->dev, "[QUP I2C Err] request_in_irq failed\n");
		...
		if (ret) {
			dev_err(&pdev->dev, "[QUP I2C Err] request_out_irq failed\n");
		...
		if (ret) {
			dev_err(&pdev->dev, "[QUP I2C Err] request_err_irq failed\n");
	...
	if (ret) {
		dev_err(&pdev->dev, "[QUP I2C Err] i2c_add_adapter failed\n");
		if (dev->num_irqs == 3) {
			free_irq(dev->out_irq, dev);
			free_irq(dev->in_irq, dev);
		}
