...
static struct clk_freq_tbl clk_tbl_vfe[] = {
	...
	F_VFE(266667000, MM_PLL1,   1, 2,  3, NOMINAL),
	F_END,
};
...
#define CLK_SRC(_src, _func, _par, _name) \
	[(_src)] = { .enable_func = (_func), .par = (_par), .name = (_name) }
struct clk_source soc_clk_sources[NUM_SRC] = {
	CLK_SRC(CXO,   xo_enable, SRC_NONE, "CXO"),
	CLK_SRC(PXO,   xo_enable, SRC_NONE, "PXO"),
	CLK_SRC(PLL_0, voteable_pll_enable, PXO, "PLL_0"),
	CLK_SRC(PLL_1, nt_pll_enable, PXO, "PLL_1"),
	CLK_SRC(PLL_2, nt_pll_enable, PXO, "PLL_2"),
	CLK_SRC(PLL_3, nt_pll_enable, PXO, "PLL_3"),
	CLK_SRC(PLL_4, pll4_enable, PXO, "PLL_4"),
	CLK_SRC(PLL_6, voteable_pll_enable, CXO, "PLL_6"),
	CLK_SRC(PLL_7, nt_pll_enable, PXO, "PLL_7"),
	CLK_SRC(PLL_8, voteable_pll_enable, PXO, "PLL_8"),
};
...
static DECLARE_BITMAP(msm_clk_soc_ignore, L_NR_CLKS);
...
int soc_clk_check_ignore(unsigned id)
{
	if (id >= L_NR_CLKS)
		return 0;

	if (bitmap_empty(msm_clk_soc_ignore, L_NR_CLKS))
		return 0;

	if (msm_clk_soc_ignore[BIT_WORD(id)] & BIT_MASK(id))
		return 1;
	else
		return 0;
...
/* Print votes for clock sources that are still held */
void soc_clk_src_votes_show()
{
	int src;
	int votes;
	int clk_count = 0;

	pr_info("Number of votes still held by source clocks:\n");
	for (src = 0; src < NUM_SRC; src++) {
		votes = local_src_get_votes(src);
		if (votes) {
			pr_info("%-5s:%2d votes still held. src = %s\n",
				soc_clk_sources[src].name, votes,
				soc_clk_sources[src].par < 0 ||
				soc_clk_sources[src].par >= NUM_SRC ? "NONE" :
				soc_clk_sources[soc_clk_sources[src].par].name);
			clk_count++;
		}
	}
	pr_info("%d source clocks still have votes held.\n", clk_count);
}
...
void msm_clk_soc_set_ignore_list(int *ignore_clk, unsigned num_ignore_clk)
{
	...
	bitmap_zero(msm_clk_soc_ignore, L_NR_CLKS);
	for (index = 0; index < num_ignore_clk; index++) {
		if (ignore_clk[index] < L_NR_CLKS)
			__set_bit(ignore_clk[index],  msm_clk_soc_ignore);
	}
}
