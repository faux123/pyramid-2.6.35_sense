...
_slumber(struct kgsl_device *device)
{
	...
	case KGSL_STATE_SLEEP:
		...
		KGSL_PWR_WARN("state -> SLUMBER, device %d\n",
				device->id);
		break;
	default:
		break;
	...
	KGSL_PWR_WARN("Done going to slumber\n");
	return status;
}
/******************************************************************/
...
_wake_from_slumber(struct kgsl_device *device)
{
	...
	KGSL_PWR_WARN("wake from slumber start\n");
	...
	KGSL_PWR_WARN("Done waking from slumber\n");
	return status;
}
