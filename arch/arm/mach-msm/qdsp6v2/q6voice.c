...
static int voice_apr_register(struct voice_data *v)
{
	...
	if (apr_mvm == NULL) {
		...
		if (apr_mvm == NULL) {
			pr_aud_err("Unable to register MVM\n");
			rc = -ENODEV;
			goto done;
		}
	...
	if (apr_cvs == NULL) {
		...
		if (apr_cvs == NULL) {
			pr_aud_err("Unable to register CVS\n");
			rc = -ENODEV;
			goto err;
		}
	...
	if (apr_cvp == NULL) {
		...
		if (apr_cvp == NULL) {
			pr_aud_err("Unable to register CVP\n");
			rc = -ENODEV;
			goto err1;
		}
...
static int voice_create_mvm_cvs_session(struct voice_data *v)
{
	...
	if (!cvs_handle) {
		if (v->voc_path == VOC_PATH_PASSIVE) {
			pr_aud_info("%s:creating CVS passive session\n", __func__);
		...
		pr_aud_info("send stream create session pkt size = %d\n",
		...
		pr_aud_info("%s: CVS create\n", __func__);
		ret = apr_send_pkt(apr_cvs, (uint32_t *) &cvs_session_cmd);
		if (ret < 0) {
			pr_aud_err("Fail in sending STREAM_CONTROL_SESSION\n");
			goto fail;
		}
		...
		if (!ret) {
			...
			pr_aud_info("%s:creating CVS full session\n", __func__);
			...
			if (ret < 0) {
				pr_aud_err("%s: Err %d sending CREATE_FULL_CTRL\n",
					   __func__, ret);
				goto fail;
			}
			ret = wait_event_timeout(v->cvs_wait,
			...
			/* Attach MVM to CVS. */
			pr_aud_info("%s: Attach MVM to stream\n", __func__);
			...
			if (ret < 0) {
				pr_aud_err("%s: Error %d sending ATTACH_STREAM\n",
				       __func__, ret);
				goto fail;
			}
			ret = wait_event_timeout(v->mvm_wait,
...
static int voice_destroy_mvm_cvs_session(struct voice_data *v)
{
	...
	if (v->voc_path == VOC_PATH_FULL) {
		pr_aud_info("%s: MVM detach stream\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending DETACH_STREAM\n",
			       __func__, ret);
		...
		ret = wait_event_timeout(v->mvm_wait,
					 ...
					 msecs_to_jiffies(TIMEOUT_MS));
		if (!ret) {
			pr_aud_err("%s: wait event timeout\n", __func__);
			goto fail;
		}

		/* Destroy CVS. */
		pr_aud_info("%s: CVS destroy session\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending CVS DESTROY\n",
			       __func__, ret);
		...
		/* Destroy MVM. */
		pr_aud_info("%s: MVM destroy session\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending MVM DESTROY\n",
			       __func__, ret);
...
static int voice_send_tty_mode_to_modem(struct voice_data *v)
{
	...
	if (tty_mode) {
		...
		pr_aud_info("tty mode =%d\n", mvm_tty_mode_cmd.tty_mode.mode);

		v->mvm_state = CMD_STATUS_FAIL;
		pr_aud_info("%s: MVM set tty\n", __func__);
		ret = apr_send_pkt(apr_mvm, (uint32_t *) &mvm_tty_mode_cmd);
		if (ret < 0) {
			pr_aud_err("Fail: sending VSS_ISTREAM_CMD_SET_TTY_MODE\n");
			goto done;
		}
		ret = wait_event_timeout(v->mvm_wait,
					 (v->mvm_state == CMD_STATUS_SUCCESS),
...
static int voice_send_cvs_cal_to_modem(struct voice_data *v)
{
	...
	cmd_buf = kzalloc((sizeof(struct apr_hdr) + BUFFER_PAYLOAD_SIZE),
								GFP_KERNEL);
	if (!cmd_buf) {
		pr_aud_err("No memory is allocated.\n");
		return -ENOMEM;
	}
	...
	for (; index < cal_data.num_cal_blocks; index++) {
		...
		if (cal_size_per_network >= BUFFER_PAYLOAD_SIZE)
			pr_aud_err("Cal size is too big\n");
		...
		if (ret < 0) {
			pr_aud_err("Fail: sending cvs cal, idx=%d\n", index);
			continue;
		}
		ret = wait_event_timeout(v->cvs_wait,
					 (v->cvs_state == CMD_STATUS_SUCCESS),
...
static int voice_send_cvp_vol_tbl_to_modem(struct voice_data *v)
{
	...
	for (; index < cal_data.num_cal_blocks; index++) {
		...
		ret = apr_send_pkt(apr_cvp, cmd_buf);
		if (ret < 0) {
			pr_aud_err("Fail: sending cvp vol cal, idx=%d\n", index);
			continue;
		}
		ret = wait_event_timeout(v->cvp_wait,
					 (v->cvp_state == CMD_STATUS_SUCCESS),
...
static int voice_config_cvs_vocoder(struct voice_data *v)
{
	...
	pr_aud_info("%s: Setting media type\n", __func__);
	...
	if (ret < 0) {
		pr_aud_err("%s: Error %d sending SET_MEDIA_TYPE\n",
		       __func__, ret);
	...
	case VSS_MEDIA_ID_EVRC_MODEM: {
		...
		pr_aud_info("%s: Setting EVRC min-max rate\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending SET_EVRC_MINMAX_RATE\n",
			       __func__, ret);
	...
	case VSS_MEDIA_ID_AMR_NB_MODEM: {
		...
		pr_aud_info("%s: Setting AMR rate\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending SET_AMR_RATE\n",
			       __func__, ret);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending SET_DTX\n",
			       __func__, ret);
	...
	case VSS_MEDIA_ID_AMR_WB_MODEM: {
		...
		pr_aud_info("%s: Setting AMR WB rate\n", __func__);
		...
		if (ret < 0) {
			pr_aud_err("%s: Error %d sending SET_AMRWB_RATE\n",
			       __func__, ret);
...
static int voice_send_start_voice_cmd(struct voice_data *v)
{
	...
	AUD_LOG("send mvm_start_voice_cmd pkt size = %d\n",
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending VSS_IMVM_CMD_START_VOICE\n");
		goto fail;
	}
	ret = wait_event_timeout(v->mvm_wait,
				 (v->mvm_state == CMD_STATUS_SUCCESS),
...
static int voice_send_stop_voice_cmd(struct voice_data *v)
{
	...
	AUD_LOG("send mvm_stop_voice_cmd pkt size = %d\n",
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending VSS_IMVM_CMD_STOP_VOICE\n");
		goto fail;
	}
	ret = wait_event_timeout(v->mvm_wait,
				 (v->mvm_state == CMD_STATUS_SUCCESS),
...
static int voice_setup_modem_voice(struct voice_data *v)
{
	...
	pr_aud_info(" send create cvp session, pkt size = %d\n",
	...
	pr_aud_info("topology=%d net_id=%d, dir=%d tx_port_id=%d, rx_port_id=%d\n",
			cvp_session_cmd.cvp_session.tx_topology_id,
...
static int voice_enable_vocproc(struct voice_data *v)
{
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending VSS_IVOCPROC_CMD_ENABLE\n");
		goto fail;
	}
	ret = wait_event_timeout(v->cvp_wait,
				 (v->cvp_state == CMD_STATUS_SUCCESS),
	...
	pr_aud_info("send mvm_a_vocproc_cmd pkt size = %d\n",
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending VSS_ISTREAM_CMD_ATTACH_VOCPROC\n");
		goto fail;
	}
	ret = wait_event_timeout(v->mvm_wait,
				 (v->mvm_state == CMD_STATUS_SUCCESS),
...
static int voice_destroy_modem_voice(struct voice_data *v)
{
	...
	pr_aud_info("mvm_d_vocproc_cmd  pkt size = %d\n",
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending VSS_ISTREAM_CMD_DETACH_VOCPROC\n");
		goto fail;
	}
	ret = wait_event_timeout(v->mvm_wait,
				 (v->mvm_state == CMD_STATUS_SUCCESS),
	...
	pr_aud_info("cvp_destroy_session_cmd pkt size = %d\n",
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending APRV2_IBASIC_CMD_DESTROY_SESSION\n");
		goto fail;
	}
	ret = wait_event_timeout(v->cvp_wait,
				 (v->cvp_state == CMD_STATUS_SUCCESS),
...
static int voice_send_vol_index_to_modem(struct voice_data *v)
{
	...
	if (ret < 0) {
		pr_aud_err("Fail in sending RX VOL INDEX\n");
		return -EINVAL;
	}
	ret = wait_event_timeout(v->cvp_wait,
				 (v->cvp_state == CMD_STATUS_SUCCESS),
			...
			void *private_data)
{
	...
	if ((evt_id != AUDDEV_EVT_START_VOICE) ||
		...
		if (evt_payload == NULL) {
			pr_aud_err(" evt_payload is NULL pointer\n");
			return;
		}
	}
...
int voice_set_voc_path_full(uint32_t set)
{
	...
	if (voice.voc_state == VOC_INIT || voice.voc_state == VOC_RELEASE) {
		...
		pr_aud_err("%s: Invalid voc path set to %d, in state %d\n",
		...
		if (data->payload_size) {
			...
			ptr[0] ==
				...
				if (!ptr[1]) {
					...
					pr_aud_info("got NACK for sending \
							MVM create session \n");
				v->mvm_state = CMD_STATUS_SUCCESS;
				wake_up(&v->mvm_wait);
				...
				pr_aud_info("%s: ATTACH_STREAM resp 0x%x\n",
					__func__, ptr[1]);
	...
	if (data->opcode == APR_BASIC_RSP_RESULT) {
		...
		if (voc_pkt != NULL && v->mvs_info.ul_cb != NULL) {
			...
			v->mvs_info.ul_cb((uint8_t *)voc_pkt,
				...
				pr_aud_err("%s: voc_pkt is 0x%x ul_cb is 0x%x\n",
		...
		if (v->mvs_info.dl_cb != NULL) {
			...
			if (ret < 0) {
				pr_aud_err("%s: Error %d sending DEC_BUF\n",
				       __func__, ret);
				goto fail;
			}
		} else {
			pr_aud_err("%s: ul_cb is NULL\n", __func__);
		}
	} else {
		...
		if (data->payload_size) {
			...
			if (ptr[0] ==
				...
				if (!ptr[1]) {
					...
					pr_aud_info("got NACK from CVP create \
						session response\n");
				v->cvp_state = CMD_STATUS_SUCCESS;
				wake_up(&v->cvp_wait);
			} else if (ptr[0] ==
