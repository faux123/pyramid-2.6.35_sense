...
static unsigned ulpi_read(struct msm_otg *dev, unsigned reg)
{
	...
	if (timeout == 0) {
		USBH_ERR("ulpi_read: timeout %08x\n",
...
/* Mark below due we use PMIC to do vbus detection. */
#if 0
...
static void msm_otg_del_timer(struct msm_otg *dev)
{
	...
	USBH_DEBUG("deleting %s timer. remaining %lld msec \n", timer_string(bit),
...
static int msm_otg_start_hnp(struct otg_transceiver *xceiv)
{
	...
	USBH_DEBUG("A-Host: HNP initiated\n");
...
static int msm_otg_start_srp(struct otg_transceiver *xceiv)
{
	...
	if ((jiffies - dev->b_last_se0_sess) < msecs_to_jiffies(TB_SRP_INIT)) {
		USBH_DEBUG("initial conditions of SRP are not met. Try again"
				"after some time\n");
		ret = -EAGAIN;
		goto out;
	}
...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	USBH_INFO("%s: usb in low power mode\n", __func__);
...
static int msm_otg_resume(struct msm_otg *dev)
{
	...
	USBH_INFO("%s: usb exited from low power mode\n", __func__);
...
static int msm_otg_set_suspend(struct otg_transceiver *xceiv, int suspend)
{
	...
	USBH_DEBUG("suspend request in state: %s\n",
			state_string(state));
			...
			struct usb_gadget *gadget)
{
	...
	USBH_INFO("peripheral driver registered w/ tranceiver\n");
...
static int msm_otg_set_host(struct otg_transceiver *xceiv, struct usb_bus *host)
{
	...
	dev->otg.host = host;
	USBH_INFO("host driver registered w/ tranceiver\n");
...
void htc_otg_set_vbus_state(int online)
{
	struct msm_otg *dev = the_msm_otg;
	enum usb_otg_state state;
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if (atomic_read(&dev->in_lpm)) {
		...
		wake_lock(&dev->wlock);
		queue_work_on(0, dev->wq, &dev->otg_resume_work);
		goto out;
	}
	...
	USBH_DEBUG("IRQ state: %s, otgsc = 0x%08x\n", state_string(state), otgsc);
	...
	if ((otgsc & OTGSC_IDIE) && (otgsc & OTGSC_IDIS)) {
		...
		if (otgsc & OTGSC_BSV) {
			...
			USBH_DEBUG("%s BSV clear\n", __func__);
			clear_bit(B_SESS_VLD, &dev->inputs);
		}
#endif
		work = 1;
	} else if (otgsc & OTGSC_DPIS) {
		USBH_DEBUG("%s: DPIS detected\n", __func__);
		writel(otgsc, USB_OTGSC);
		...
		case OTG_STATE_A_SUSPEND:
			if (dev->otg.host->b_hnp_enable && (pc & PORTSC_CSC) &&
					!(pc & PORTSC_CCS)) {
				USBH_DEBUG("%s B_CONN clear\n", __func__);
				clear_bit(B_CONN, &dev->inputs);
			}
			break;
		case OTG_STATE_B_WAIT_ACON:
			if ((pc & PORTSC_CSC) && (pc & PORTSC_CCS)) {
				USBH_DEBUG("%s A_CONN set\n", __func__);
...
/*   This will cause ID pin unnormal. ID pin interrupt will trigger after this function is called.
...
/* Mark below due we use PMIC to do vbus detection. */
	...
	if (dev->otg.gadget) {
		...
		if (is_b_sess_vld() && !test_bit(B_SESS_VLD, &dev->inputs)) {
			USBH_DEBUG("otg_reset: handle missing BSV event\n");
			set_bit(B_SESS_VLD, &dev->inputs);
			work = 1;
	...
	if (dev->otg.host && !dev->pmic_id_notif_supp) {
		...
		if (is_host() && test_bit(ID, &dev->inputs)) {
			USBH_DEBUG("otg_reset: handle missing ID_GND event\n");
			clear_bit(ID, &dev->inputs);
			work = 1;
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		...
		if (!test_bit(ID, &dev->inputs) ||
			...
			USBH_DEBUG("b_sess_end && b_bus_req\n");
	...
	case OTG_STATE_B_SRP_INIT:
		if (!test_bit(ID, &dev->inputs) ||
				...
				(test_bit(B_SESS_VLD, &dev->inputs) &&
				!test_bit(ID_B, &dev->inputs))) {
			USBH_DEBUG("!id || id_a/c || b_sess_vld+!id_b\n");
			msm_otg_del_timer(dev);
			spin_lock_irq(&dev->lock);
			dev->otg.state = OTG_STATE_B_IDLE;
			spin_unlock_irq(&dev->lock);
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &dev->inputs) ||
				...
				test_bit(ID_B, &dev->inputs) ||
				!test_bit(B_SESS_VLD, &dev->inputs)) {
			USBH_DEBUG("!id  || id_a/b || !b_sess_vld\n");
			clear_bit(B_BUS_REQ, &dev->inputs);
			spin_lock_irq(&dev->lock);
			dev->otg.state = OTG_STATE_B_IDLE;
			spin_unlock_irq(&dev->lock);
			...
			USBH_DEBUG("b_bus_req && b_hnp_en && a_bus_suspend\n");
			...
			USBH_DEBUG("chg_type == wallcarger\n");
#ifdef CONFIG_USB_MSM_ACA
	...
	case OTG_STATE_A_IDLE:
		...
		if (test_bit(ID, &dev->inputs) &&
			...
			USBH_DEBUG("!a_bus_drop && (a_srp_det || a_bus_req)\n");
			...
			USBH_DEBUG("No session requested\n");
	...
	case OTG_STATE_A_WAIT_VRISE:
		if ((test_bit(ID, &dev->inputs) &&
			...
			USBH_DEBUG("id || a_bus_drop || a_wait_vrise_tmout\n");
			clear_bit(A_BUS_REQ, &dev->inputs);
			msm_otg_del_timer(dev);
	...
	case OTG_STATE_A_WAIT_BCON:
		if ((test_bit(ID, &dev->inputs) &&
			...
			USBH_DEBUG("id_f/b/c || a_bus_drop ||"
					"a_wait_bcon_tmout\n");
	...
	case OTG_STATE_A_PERIPHERAL:
		if ((test_bit(ID, &dev->inputs) &&
				!test_bit(ID_A, &dev->inputs)) ||
				test_bit(A_BUS_DROP, &dev->inputs)) {
			USBH_DEBUG("id _f/b/c || a_bus_drop\n");
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	if (dev->pdata->rpc_connect) {
		...
		USBH_INFO("%s: rpc_connect(%d)\n", __func__, ret);
		if (ret) {
			USBH_ERR("%s: rpc connect failed\n", __func__);
			ret = -ENODEV;
			goto free_dev;
		}
	}
	...
	if (IS_ERR(dev->hs_clk)) {
		USBH_ERR("%s: failed to get usb_hs_clk\n", __func__);
	...
	if (!dev->pdata->pclk_is_hw_gated) {
		...
		if (IS_ERR(dev->hs_pclk)) {
			USBH_ERR("%s: failed to get usb_hs_pclk\n", __func__);
	...
	if (dev->pdata->core_clk) {
		...
		if (IS_ERR(dev->hs_cclk)) {
			USBH_ERR("%s: failed to get usb_hs_core_clk\n", __func__);
	...
	if (!dev->pdata->phy_reset) {
		...
		if (IS_ERR(dev->phy_reset_clk)) {
			USBH_ERR("%s: failed to get usb_phy_clk\n", __func__);
	...
	if (!res) {
		USBH_ERR("%s: failed to get platform resource mem\n", __func__);
		ret = -ENODEV;
		goto put_phy_clk;
	}
	...
	if (!dev->regs) {
		USBH_ERR("%s: ioremap failed\n", __func__);
		ret = -ENOMEM;
		goto put_phy_clk;
	}
	dev->irq = platform_get_irq(pdev, 0);
	if (!dev->irq) {
		USBH_ERR("%s: platform_get_irq failed\n", __func__);
		ret = -ENODEV;
		goto free_regs;
	}
	...
	ret = msm_xo_mode_vote(dev->xo_handle, MSM_XO_MODE_ON);
	if (ret) {
		USBH_ERR("%s failed to vote for TCXO"
			"D1 buffer%d\n", __func__, ret);
		goto free_xo_handle;
	}
	...
	if (dev->pdata->init_gpio) {
		...
		if (ret) {
			USBH_ERR("%s: gpio init failed with err:%d\n",
					__func__, ret);
			goto free_wq;
		}
	}
	...
	if (dev->pdata->pmic_vbus_notif_init) {
		...
		if (!ret) {
			...
			USBH_ERR("%s: pmic_vbus_notif_init() failed, err:%d\n",
					__func__, ret);
			goto free_gpio;
		}
	}
	...
	if (dev->pdata->ldo_init) {
		...
		if (ret) {
			USBH_ERR("%s: ldo_init failed with err:%d\n",
					__func__, ret);
	...
	if (dev->pdata->ldo_enable) {
		...
		if (ret) {
			USBH_ERR("%s: ldo_enable failed with err:%d\n",
					__func__, ret);
			goto free_ldo_init;
		}
	}
	...
	ret = request_irq(dev->irq, msm_otg_irq, IRQF_SHARED,
					"msm_otg", dev);
	if (ret) {
		USBH_INFO("%s: request irq failed\n", __func__);
...
/* Mark these code due to this will cause suspend resume from TCXO USB
cannot connect.
static int msm_otg_runtime_suspend(struct device *dev)
{
	struct msm_otg *otg = the_msm_otg;

	USBH_DEBUG("pm_runtime: suspending...\n");
	msm_otg_suspend(otg);
	return  0;
}
...
static int msm_otg_runtime_resume(struct device *dev)
{
	...
	USBH_DEBUG("pm_runtime: resuming...\n");
	msm_otg_resume(otg);
	return  0;
}
