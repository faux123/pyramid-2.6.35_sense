...
static irqreturn_t msm_spi_error_irq(int irq, void *dev_id)
{
	...
	if (spi_err & SPI_ERR_INPUT_OVER_RUN_ERR) {
		 ...
		 */
		if (dd->mode != SPI_MODE_NONE && dd->read_buf)
...
static void msm_spi_process_transfer(struct msm_spi *dd)
{
	...
	if (dd->cur_msg->spi->ext_gpio_cs)
		gpio_set_value(dd->cur_msg->spi->ext_gpio_cs, 0);
	...
	*/
	...
	do {
		if (!wait_for_completion_timeout(&dd->transfer_complete,
						 timeout)) {
				dev_err(dd->dev, "QUP_SPI_ERR: %s: SPI transaction "
						 "timeout\n", __func__);
				dd->cur_msg->status = -EIO;
				if (dd->mode == SPI_DMOV_MODE) {
...
transfer_end:
	...
	writel_relaxed(spi_ioc & ~SPI_IO_C_MX_CS_MODE, dd->base + SPI_IO_CONTROL);

	if (dd->cur_msg->spi->ext_gpio_cs)
		gpio_set_value(dd->cur_msg->spi->ext_gpio_cs, 1);
}
...
//	u32 cs_wait = readl(dd->base + SPI_DEASSERT_WAIT);
//	writel( 0x3 | cs_wait, dd->base + SPI_DEASSERT_WAIT);
...
static int msm_spi_transfer(struct spi_device *spi, struct spi_message *msg)
{
	...
	list_for_each_entry(tr, &msg->transfers, transfer_list) {
		...
		if (tr->speed_hz > dd->pdata->max_clock_speed ||
			...
			dev_err(&spi->dev, "QUP_SPI_ERR: Invalid transfer: %d Hz, %d bpw"
					   "tx=%p, rx=%p\n",
		...
		do {
			if (tx_buf != NULL) {
				...
				if (dma_mapping_error(NULL, tr->tx_dma)) {
					dev_err(&spi->dev,
						"QUP_SPI_ERR: dma %cX %d bytes error\n",
						'T', len);
					goto error;
				}
			}
...
static int msm_spi_setup(struct spi_device *spi)
{
	...
	if (spi->bits_per_word < 4 || spi->bits_per_word > 32) {
		dev_err(&spi->dev, "QUP_SPI_ERR: %s: invalid bits_per_word %d\n",
			__func__, spi->bits_per_word);
		rc = -EINVAL;
	}
	if (spi->chip_select > SPI_NUM_CHIPSELECTS-1) {
		dev_err(&spi->dev, "QUP_SPI_ERR: %s, chip select %d exceeds max value %d\n",
				      ...
				      struct msm_dmov_errdata *err)
{
	...
	if (result & DMOV_RSLT_DONE)
		...
		if (result & DMOV_RSLT_ERROR) {
			dev_err(dd->dev, "QUP_SPI_ERR: DMA error (0x%08x)\n", result);
			dd->stat_dmov_tx_err++;
		}
		...
		if (err)
			dev_err(dd->dev,
				"QUP_SPI_ERR: Flush data(%08x %08x %08x %08x %08x %08x)\n",
...
static int __init msm_spi_probe(struct platform_device *pdev)
{
	...
	if (!master) {
		rc = -ENOMEM;
		dev_err(&pdev->dev, "QUP_SPI_ERR: master allocation failed\n");
		goto err_probe_exit;
	}
...
skip_dma_resources:
		if (pdata->gpio_config) {
			...
			if (rc) {
				dev_err(&pdev->dev,
					"QUP_SPI_ERR: %s: error configuring GPIOs\n",
					__func__);
				goto err_probe_gpio;
			}
		}
	...
	if (pdata && pdata->rsl_id) {
		...
		if (rc) {
			dev_err(&pdev->dev, "QUP_SPI_ERR: %s: unable to init remote_mutex "
				"(%s), (rc=%d)\n", rmid.r_spinlock_id,
				__func__, rc);
		...
		if (IS_ERR(qos_req_list)) {
			dev_err(&pdev->dev, "QUP_SPI_ERR: %s: Failed to request pm_qos\n",
				__func__);
	...
	if (IS_ERR(dd->clk)) {
		dev_err(&pdev->dev, "QUP_SPI_ERR: %s: unable to get %s\n", __func__,
			pdata->clk_name);
		rc = PTR_ERR(dd->clk);
	...
	if (rc) {
		dev_err(&pdev->dev, "QUP_SPI_ERR: %s: unable to enable %s\n",
	...
	if (rc) {
		dev_err(&pdev->dev, "QUP_SPI_ERR: failed to create dev. attrs : %d\n", rc);
		goto err_attrs;
	}
